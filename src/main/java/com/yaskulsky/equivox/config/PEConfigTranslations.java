package com.yaskulsky.equivox.config;
...
public enum PEConfigTranslations implements IConfigTranslation {
	...
	CLIENT_TOOLTIPS("client.tooltips", "Tooltip Settings", "Settings for configuring Tooltips provided by Equivox.", true),
	...
	CLIENT_TOOLTIPS_STATS("client.tooltips.stats", "Stat Tooltips", "Show stats as tooltips for various Equivox blocks."),
	...
	COMMON_CRAFTING("common.crafting", "Crafting Settings", "Settings for configuring crafting requirements of specific Equivox recipes.", true),
	...
	SERVER_COOLDOWN("server.cooldown", "Cooldown Settings",
			"Settings for configuring the Cooldown (in ticks) for various features in Equivox. "
	...
	SERVER_DIFFICULTY("server.difficulty", "Difficulty Settings", "Settings for configuring Difficulty options provided by Equivox.", true),
	...
	SERVER_ITEMS("server.items", "Item Settings", "Settings for configuring Item options provided by Equivox.", true),
	SERVER_ITEMS_PICKAXE_AOE_VEIN_MINING("server.items.pickaxe_aoe_vein_mining", "Pickaxe AOE Vein Mining",
			...
			+ "like it did in Equivox before version 1.4.4."),
	...
	SERVER_MISC("server.misc", "Misc Settings", "Settings for configuring misc options provided by Equivox.", true),
	...
	MAPPING_DUMP_TO_FILE("mapping.dump_to_file", "Dump Everything To File",
			"Want to take a look at the internals of EMC Calculation? Enable this to write all the conversions and setValue-Commands to config/Equivox/mapping_dump.json"),
	MAPPING_PREGENERATED("mapping.pregenerated", "Pregenerate EMC",
			"When the next EMC mapping occurs write the results to config/Equivox/pregenerated_emc.json and only ever run the mapping again when that file "
	...
	MAPPING_LOG_EXPLOITS("mapping.log_exploits", "Log Known EMC Exploits",
			...
			+ "that the algorithm did not overwrite. Exploits that derive from conversions that are unknown to Equivox will not be found."),
