...
public class EntityWaterProjectile extends NoGravityThrowableProjectile {
	...
	public void tick() {
		...
		if (!level.isClientSide() && isAlive()) {
			...
			if (getY() > level.getMaxY() && level instanceof ServerLevel serverLevel) {
	...
	protected void onHitBlock(@NotNull BlockHitResult result) {
		...
		if (!level().isClientSide() && getOwner() instanceof Player player) {
			WorldHelper.placeFluid(player, level(), result.getBlockPos(), result.getDirection(), Fluids.WATER, !EquivoxConfig.server.items.opEvertide.get());
		}
	}
