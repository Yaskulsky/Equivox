...
public class PEPermissions {
	...
	private static final PermissionResolver<Boolean> PLAYER_IS_OP = (player, uuid, context) -> player != null && player.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER);
	...
	private static CommandPermissionNode nodeSubCommand(CommandPermissionNode parent, String nodeName) {
		...
		return new CommandPermissionNode(node, parent.fallbackPermission);
	}
	...
	public record CommandPermissionNode(PermissionNode<Boolean> node, PermissionCheck fallbackPermission) implements Predicate<CommandSourceStack> {
		...
		public boolean test(CommandSourceStack source) {
			...
			return fallbackPermission.check(source.permissions()) || source.getEntity() instanceof ServerPlayer player && PermissionAPI.getPermission(player, node);
		}
	}
