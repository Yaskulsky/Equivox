...
import dev.shadowsoffire.apotheosis.Apoth.Triggers;
...
public class AdventureEvents {
   ...
   public void fireProjectile(EntityJoinLevelEvent e) {
      if (e.getEntity() instanceof Projectile proj
         && !proj.getPersistentData().getBooleanOr("apoth.generated", false)
         && proj.getOwner() instanceof LivingEntity user) {
   ...
   public void impact(ProjectileImpactEvent e) {
      Projectile affixes = e.getProjectile();
      if (affixes instanceof Projectile) {
         SocketHelper.getGemInstances(affixes).forEach(inst -> inst.onProjectileImpact(affixes, e.getRayTraceResult()));
         Map<DynamicHolder<Affix>, AffixInstance> affixesx = AffixHelper.getAffixes(affixes);
         affixesx.values().forEach(inst -> inst.onProjectileImpact(affixes, e.getRayTraceResult(), e.getRayTraceResult().getType()));
      }
   }
   ...
   public void equip(LivingEquipmentChangeEvent e) {
      if (e.getEntity() instanceof ServerPlayer player) {
         Triggers.EQUIPPED_ITEM.trigger(player, e.getSlot(), e.getTo());
      }
   }
   ...
   public void applyMissedTierAugments(EntityJoinLevelEvent e) {
      if (e.getLevel() instanceof ServerLevelAccessor) {
         Entity entity = e.getEntity();
         if (!(Boolean)entity.getData(Attachments.TIER_AUGMENTS_APPLIED)) {
            if (entity instanceof Player player) {
               WorldTier tier = (WorldTier)player.getData(Attachments.WORLD_TIER);
   ...
   public void sync(OnDatapackSyncEvent e) {
      ...
      e.sendRecipes(new RecipeType[]{RecipeTypes.SALVAGING, RecipeTypes.GEM_CUTTING});
   ...
   public void recordColdDamage(net.neoforged.neoforge.event.entity.living.LivingDamageEvent.Post e) {
      if (e.getSource().is(dev.shadowsoffire.apothic_attributes.api.ALObjects.DamageTypes.COLD_DAMAGE)) {
         LivingEntity entity = e.getEntity();
         entity.setData(Attachments.COLD_DAMAGE_TAKEN, (Float)entity.getData(Attachments.COLD_DAMAGE_TAKEN) + e.getInflictedDamage());
      }
   }
   ...
   public void syncRadialState(EntityJoinLevelEvent e) {
      if (e.getEntity() instanceof ServerPlayer player) {
         PacketDistributor.sendToPlayer(player, new RadialStatePayload(RadialState.getState(player)), new CustomPacketPayload[0]);
      }
   }
}
